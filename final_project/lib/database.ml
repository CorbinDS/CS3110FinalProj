...
(* [item_of_json json] converts [json] to ADT of the items of the menus
   of eateries on the net nutrition website *)
...
(* [menu_of_json json] converts [json] to ADT of the menus of eateries
...
(* [eatery_of_json json] converts [json] to ADT of the eateries on the
   net nutrition website *)
...
let nn_eatery_names () =
  List.map get_eatery_name (get_net_nutrition ()).eateries
...
(** [menu_items_from_json station] maps the menu items from the json
    into (station, items) format. *)
...
(** [dining_hall_from_json json] maps the dining halls from the json
    into the dining hall record type. *)
...
(** [menu_from_json json] maps the menu from the json into the menu
    record type. *)
...
(*[edit_menu f nn] edits the corresponding dining hall json to match the
  information in [nn]. It does this by converting the json file [f] to a
  string and replacing the menu_items information with information
  obtained from its helper function replacement_yj. After fixing the
  string to maintain Yojson's compact string formatting, we can
  overwrite the original json file to reflect the new information. *)
let edit_menu (mn : m) (nn : nn_type) =
  add_m_to_file
    {
      eatery = mn.eatery;
      ...
      menu_items =
        [
          ( "",
            List.concat
              (List.map
                 (fun m -> List.map (fun (i : item) -> i.name) m.items)
                 (get_eatery_by_name mn.eatery.name nn).menus) );
        ];
    }
    (String.map
       (fun c -> if c = ' ' then '_' else c)
       (mn.menu_name ^ "_" ^ mn.eatery.name))

(*[check_and_edit_menu nn f] checks if f has a nn page. If it does, it
  calls its helper function: edit_menu.*)
let check_and_edit_menu (nn : nn_type) (mn : m) =
  if List.mem mn.eatery.name (nn_eatery_names ()) then edit_menu mn nn

(*[edit_menu_jsons nn] updates all the menu json files in our database
  that have corresponding net nutrition pages to reflect the information
  in our nn data type. This means filtering done to the nn data type
  will be reflected in the corresponding menu jsons, which will be
...
(** [dininginfo ()] is the eateries webpage converted into a string list
    list of separated by dining hall. For instance, \[[all menus for
    dining hall 1],[all menus for dining hall 2],...] *)
let dininginfo () =
...
(** [get_menu_items menu] maps the list in the format \[station, item1,
    item2,...]\ into [(station, \[items]\)]*)
...
(** [web_into_d hallinfo] takes a dining hall from the webpage and maps
    it into the dining hall record type. *)
...
(** [web_into_m_helper hallinfo hours hall_menus menu_types] is a helper
    function for [web_into_m_list hallinfo] *)
...
(** [web_into_m_list hallinfo] takes the menus specified for a singular
    dining hall [hallinfo] from the webpage and converts into a list of
    menus.*)
...
(** [update_nutritional_information ()] updates the nutritional
    information in the database by pulling from the website
...
let update_nutritional_information () =
  ...
  |> fun x ->
  print_endline "Finished scraping.";
...
(** [update_menus ()] updates the menu information in the database by
    pulling from the eateries website. *)
let update_menus () =
  Sys.chdir "database";
...
(* Shows dining halls where dining hall name passed in is contained in
   any of the official dining hall names.*)
...
(* Shows dining halls where the campus location passed in is contained
   in any of the campus locations.*)
...
(** Shows dining halls where the contact passed in is an exact match
    with any of the dining hall contact numbers.*)
...
(** Shows the dining halls that are open during the time period passed
    in (whether strictly within the dining hall operating hours or
    partially within the dining hall operating hours). *)
...
let rec filter_dining_halls
  ...
  | Dining_Name n :: t ->
      filter_dining_halls t (filter_dining_hall_by_name n ds)
  | Campus_Location l :: t ->
      filter_dining_halls t (filter_dining_hall_by_campus_location l ds)
  | Contact c :: t ->
      filter_dining_halls t (filter_dining_hall_by_contact c ds)
  | Open_During (o, e, s) :: t ->
      filter_dining_halls t (filter_dining_hall_by_time_range o e s ds)
  | Description des :: t ->
      filter_dining_halls t (filter_dining_hall_by_description des ds)
...
let rec net_nutrition_filterer (ing : string list) (nn : nn_type) :
    nn_type =
  match ing with
  | [] -> nn
  | h :: t ->
...
let ingredient_applicable_menus (ms : m list) =
  List.filter (fun m -> List.mem m.eatery.name (nn_eatery_names ())) ms

(* List.map (fun (i : item) -> print_endline i.name) (List.hd (List.hd
   (filtered_nn.eateries)).menus).items; *)
let apply_ingredient_filter (ings : string list) (ms : m list) : m list
    =
  let filtered_nn =
    net_nutrition_filterer ings (get_net_nutrition ())
  in
  let ing_applicable_menus = ingredient_applicable_menus ms in
  let rec filtering_process
      (f_nn : n)
      (i_filt_menus : m list)
      (acc : m list) =
                         ...
                         (List.map
                            (fun m ->
                              List.map
                                (fun (i : item) -> i.name)
                                m.items)
                            (get_eatery_by_name h.eatery.name f_nn)
                              .menus) );
                   ];
               };
             ])
...
let apply_eatery_filter (halls : d list) (mn : m) =
  List.exists (fun hall -> hall = mn.eatery) halls
...
let apply_open_filter (o : int) (e : int) (s : in_range_spec) (mn : m) =
  try
    match s with
    | StrictlyWithinRange ->
        in_time_range (List.nth mn.hours 0) (List.nth mn.hours 1) o
        && in_time_range (List.nth mn.hours 0) (List.nth mn.hours 1) e
    | PartiallyWithinRange ->
        in_time_range (List.nth mn.hours 0) (List.nth mn.hours 1) o
        || in_time_range (List.nth mn.hours 0) (List.nth mn.hours 1) e
  with Failure x -> false

let apply_item_filter (i : string) (mn : m) =
  List.exists
    (fun (station, items) ->
      List.exists
        (fun it ->
        ...
        items)
    mn.menu_items
  && List.length mn.menu_items >= 1

let apply_avoid_filter (i : string) (mn : m) =
  List.for_all
    (fun (station, items) ->
      List.for_all
        (fun it ->
...
let rec filter_menus (attr : menu_attributes list) (ms : m list) :
  ...
  | Eateries halls :: t ->
      ...
      filter_menus t (List.filter (apply_eatery_filter halls) ms)
  | Menu_Name name :: t ->
      (* Shows menus where the menu name passed in is contained in any
         of the official menu names.*)
      filter_menus t (List.filter (apply_menu_name_filter name) ms)
  | Open_During (o, e, s) :: t ->
      (* Shows the menus that are available during the time period
         passed in (whether strictly within the menu availability hours
         or partially within the menu availability hours). *)
      filter_menus t (List.filter (apply_open_filter o e s) ms)
  | Item i :: t ->
      (* Shows the menus where the item passed in is on the menu. *)
      filter_menus t (List.filter (apply_item_filter i) ms)
  | Avoid i :: t ->
      (* Shows the menus where the item passed in is not on the menu.*)
      filter_menus t (List.filter (apply_avoid_filter i) ms)
  | Ingredient s :: t ->
      let tempy =
        apply_ingredient_filter (String.split_on_char ',' s) ms
      in
      filter_menus t tempy
